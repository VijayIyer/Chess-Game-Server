...
def create_piece_per_conf(piece_pos):
    piece_col, piece_row =   8 - int(piece_pos[1]),\
        8 - (104 - ord(piece_pos[0])) - 1
    if piece_pos[3] == "W":
        color = "white"
    else: color = "black"
    
    if piece_pos == "Q": piece = pawn(color, current_pos=(piece_row, piece_col))
    if piece_pos == "R": piece = rook(color, current_pos=(piece_row, piece_col))
    if piece_pos == "N": piece = knight(color, current_pos=(piece_row, piece_col))
    if piece_pos == "K": piece = king(color, current_pos=(piece_row, piece_col))
    if piece_pos == "B": piece = bishop(color, current_pos=(piece_row, piece_col))
    else: piece = pawn(color, current_pos=(piece_row, piece_col))
    return piece
